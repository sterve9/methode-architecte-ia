...
| Isolation des données entre utilisateurs | Aucun test ne la mesure. Le système n'a qu'un seul compte (`DT-Lot1-01`) : il n'existe aucun projet d'un autre utilisateur à tenter d'atteindre, la mesure est donc impossible aujourd'hui. | Les policies RLS `user_id = auth.uid()` sont en place et ont été relues en S24. La vérification repose sur cette **lecture**, pas sur une mesure. **À refaire le jour où un second compte existera.** |
